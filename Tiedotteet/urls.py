...
urlpatterns = [
    ...
    url(r'^cp/messages/(?P<pk>\d+)/edit/$',
        views.edit_message, name="edit"),
    url(r'^cp/messages/(?P<pk>\d+)/delete/$', views.delete_message),
    url(r'^cp/messages/(?P<pk>\d+)/hide/$',
        views.hide_message, name="delete"),
    url(r'^cp/messages/(?P<filter>\w+)/(?P<category>\w+)/$',
